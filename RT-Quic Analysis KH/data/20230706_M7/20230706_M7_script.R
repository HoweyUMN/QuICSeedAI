...
#read files 
plate = read_xlsx('20230706_M7_plate.xlsx')
raw = read_xlsx('20230706_M7_raw.xlsx')
replicate = read_xlsx('20230706_M7_replicate.xlsx')
...
Instrument_number <- str_extract('20230706_M7_replicate', "[A-Z]\\d+") #str_extract(file_names, "[T]\\d+") 
...
# PlotRawMulti_new(clean_raw.65, samples)
...
stats_norm = GetStats(norm_analysis_spread, 'neg', test = 'wilcox', tail = 'greater', alpha = 0.1) 
stats_norm_new = GetStats(norm_analysis_spread, 'neg', test = 'yuen', tail = 'greater', adjust_p = TRUE)
